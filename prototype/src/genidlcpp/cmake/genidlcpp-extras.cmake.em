...
macro(_generate_srv_idlcpp ARG_PKG ARG_SRV ARG_IFLAGS ARG_MSG_DEPS ARG_GEN_OUTPUT_DIR)
  ...
  _generate_msg_idlcpp_dds(${ARG_PKG}
    "${SRV_SHORT_NAME}"
    "${ARG_IFLAGS}"
    "${ARG_MSG_DEPS};${CATKIN_DEVEL_PREFIX}/${CATKIN_GLOBAL_SHARE_DESTINATION}/${ARG_PKG}/dds_impl/${SRV_SHORT_NAME}Request.msg;${CATKIN_DEVEL_PREFIX}/${CATKIN_GLOBAL_SHARE_DESTINATION}/${ARG_PKG}/dds_impl/${SRV_SHORT_NAME}Response.msg"
    ${ARG_GEN_OUTPUT_DIR})
...
macro(_generate_msg_idlcpp_dds ARG_PKG MSG_SHORT_NAME ARG_IFLAGS ARG_MSG_DEPS ARG_GEN_OUTPUT_DIR)
  ...
  set(COMMAND "${GENIDLCPP_PP_BIN}" -I "${CATKIN_DEVEL_PREFIX}/${CATKIN_GLOBAL_SHARE_DESTINATION}" -I "@(CATKIN_DEVEL_PREFIX)/@(CATKIN_GLOBAL_SHARE_DESTINATION)" -S -l cpp -o dds-types -d "${ARG_GEN_OUTPUT_DIR}/dds_impl" "${IDL_INPUT_FILE}")
...
macro(_generate_msg_idlcpp_convert ARG_PKG ARG_MSG ARG_IFLAGS ARG_MSG_DEPS ARG_GEN_OUTPUT_DIR)
  ...
  elseif(${MSG_EXT} STREQUAL ".srv")
    set(GEN_OUTPUT_FILES
      "${ARG_GEN_OUTPUT_DIR}/impl/${MSG_SHORT_NAME}Request_pubsub.hpp"
      "${ARG_GEN_OUTPUT_DIR}/impl/${MSG_SHORT_NAME}Request_pubsub.cpp"
      "${ARG_GEN_OUTPUT_DIR}/impl/${MSG_SHORT_NAME}Response_pubsub.hpp"
      "${ARG_GEN_OUTPUT_DIR}/impl/${MSG_SHORT_NAME}Response_pubsub.cpp"
    )
  else()
...
macro(_generate_module_idlcpp ARG_PKG ARG_GEN_OUTPUT_DIR)
  ...
  foreach(dep ${${ARG_PKG}_MSG_DEPENDENCIES})
    ...
    include_directories(BEFORE ${CATKIN_DEVEL_PREFIX}/include/${dep}/impl)
    list(APPEND dds_impl_libs ${dep}_msgs)
  endforeach()
  if(NOT "${ARGN}" STREQUAL "")
    add_library(${PROJECT_NAME}_msgs ${ARGN})
    ...
    # it.
    add_dependencies(${PROJECT_NAME}_msgs ${PROJECT_NAME}_genidlcpp)
    target_link_libraries(${PROJECT_NAME}_msgs ${dds_impl_libs} ${OPENSPLICE_LIBRARIES} ${rclcpp_LIBRARIES})
    list(APPEND ${PROJECT_NAME}_LIBRARIES ${PROJECT_NAME}_msgs)
  endif()
endmacro()
